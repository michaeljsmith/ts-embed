...
type MaybeCallableExpression<T> =
    T extends Lambda<infer I, infer O>
...
export type ReferenceExpression<T> =
    MaybeCallableExpression<T> & {
      expressionType: "reference";
      argumentKey: number;
    }

export type CallExpression<O> =
    MaybeCallableExpression<O> & {
      expressionType: "call";
      ...
      visit: <R>(visitor: <I>(fnExpression: Expression<Lambda<I, O>>, argExpression: Expression<I>) => R) => R;
    }
...
export function lambda<I, O>(bodyFn: (input: Expression<I>) => Expression<O>)
  ...
  const referenceExpression = (input: Expression<I>): Expression<O> => call(referenceExpression, input);
  referenceExpression.expressionType = "reference" as "reference";
  referenceExpression.argumentKey = argumentKey;
  const expression: Expression<O> = bodyFn(referenceExpression as Expression<I>);

  // Make the result callable in case it is a lambda type.
  const result = (input: Expression<I>): Expression<O> => call(result as LambdaExpression<I, O>, input);
...
function call<I, O>(fnExpression: Expression<Lambda<I, O>>, argExpression: Expression<I>)
      : Expression<O> {

  function callExpressionHost<I, O>(fnExpression: Expression<Lambda<I, O>>, argExpression: Expression<I>)
        : <R>(visitor: <I>(fnExpression: Expression<Lambda<I, O>>, argExpression: Expression<I>) => R) => R {
    return visitor => visitor(fnExpression, argExpression);
  }

  const result = (input: Expression<any>) => call(fnExpression, argExpression);
  result.expressionType = "call" as "call";
  result.visit = callExpressionHost(fnExpression, argExpression);
  return result as CallExpression<O>;
}
